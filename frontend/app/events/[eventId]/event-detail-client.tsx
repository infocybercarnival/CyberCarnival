...
export function EventDetailClient({ eventId }: EventDetailClientProps) {
  ...
  // Find static fallback using central getEventBySlug helper
  const staticFallback = getEventBySlug(eventId)


  // Fetch event data & backend events list asynchronously in background
  useEffect(() => {
    setNotFound(false)
    if (staticFallback) {
      setLoading(false)
    ...
    fetchEvent(eventId)
      ...
      .catch((err) => {
        if (err?.status === 404 && !staticFallback) {
          setNotFound(true)
        }
        setLoading(false)
      })
  ...
  const isOpen = event ? event.registration_open !== false : true
