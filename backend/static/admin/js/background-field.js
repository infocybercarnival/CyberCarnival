/**
 * Global CyberCarnival Atmospheric Background & Spider Animation Canvas
 * Faithful 1-to-1 JavaScript port of frontend/components/background-field.tsx
 */
(function () {
  'use strict';
  ...
  function buildScene() {
    var hues = ['violet', 'indigo', 'magenta'];
    nebulas = [];
    ...
    var starCount = Math.round(((width * height) / 1000000) * STAR_COUNT_PER_MPX);
    stars = [];
    ...
    var spiderCount = width >= 1024 ? 16 : 9;
    var baseBodySize = width >= 1024 ? Math.max(7, Math.min(10, width * 0.007)) : 5.5;

    spiders = [];
    for (var id = 0; id < spiderCount; id++) {
      spiders.push({
        id: id,
  ...
  function drawNebulas() {
    ...
    nebulas.forEach(function (n) {
      ...
      var rgb = NEBULA_COLORS[n.hue];
      var grad = ctx.createRadialGradient(n.x, n.y, 0, n.x, n.y, n.r);
      grad.addColorStop(0, 'rgba(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ',' + n.alpha + ')');
      grad.addColorStop(0.6, 'rgba(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ',' + (n.alpha * 0.35) + ')');
  ...
  function drawStars(tSec) {
    stars.forEach(function (s) {
      var twinkle = reduceMotion
        ? s.baseAlpha
        : s.baseAlpha * (0.55 + 0.45 * Math.sin(tSec * s.twinkleSpeed + s.twinklePhase));
      ctx.beginPath();
  ...
  function drawSpider(s, tSec) {
    ...
    var currentAlpha = s.opacity * edgeFade;
    ...
    var sz = s.size;
    var bodyBob = Math.sin(tSec * 2.5 + s.phase) * sz * 0.03;
    ...
    legConfigs.forEach(function (cfg) {
      var legSway = Math.sin(tSec * 2.8 + s.phase + cfg.phaseOffset) * 0.06;

      [-1, 1].forEach(function (side) {
        var sX = side;
        var startX = sX * sz * 0.24;
        var startY = cfg.attachY;

        var coxaX = startX + sX * sz * cfg.coxaX * 0.4;
        var coxaY = startY + sz * cfg.coxaY * 0.4;

        var femurX = startX + sX * sz * (cfg.femurX + legSway);
        var femurY = startY + sz * (cfg.femurY - Math.abs(legSway));

        var tibiaX = startX + sX * sz * (cfg.tibiaX + legSway * 0.8);
        var tibiaY = startY + sz * (cfg.tibiaY + legSway * 0.5);

        var tarsusX = startX + sX * sz * (cfg.tarsusX + legSway * 0.5);
        var tarsusY = startY + sz * cfg.tarsusY;
